...
# Team ID:			[ 2433 ]
# Author List:		[ Harshvardhan,Bhargav,Saksham,Rahul ]
# Filename:			task_2b.py
...
def get_vision_sensor_image():
	...
	##############	ADD YOUR CODE HERE	##############
	ret,handle=sim.simxGetObjectHandle(client_id,'Vision_sensor',sim.simx_opmode_blocking)
...
def send_data(maze_array):
	...
	for i in range(10):
		for j in range(10):
			inputInts.append(int(np.binary_repr(maze_array[i][j])))
			#print(inputInts[cnt])
			cnt=cnt+1
