...
def applyPerspectiveTransform(input_img):
	...
	_, gray = cv2.threshold(gray, 240, 255, cv2.THRESH_TOZERO)
	...
	# This block of code is to find the coordinates of corners of the maze, so that we can find the warped image.
	for i in corners:
		global r
		x,y=i.ravel()
		dd1=(tx-x)**2+(ty-y)**2
		...
		if dd4 > d4:
			...
			cx4 = x
			cy4 = y
	if ((cx1-cx2)**2+(cy1-cy2))**2>rows*rows/16 and ((cx1-cx3)**2+(cy1-cy3))**2>rows*rows/16:
		pts1 = np.float32([[cx4, cy4], [cx2, cy2], [cx3, cy3], [cx1, cy1]])
		pts2 = np.float32([[0, 0], [rows, 0], [0, rows], [rows, rows]])

		matrix = cv2.getPerspectiveTransform(pts1, pts2)
		r = rows
		warped_img = cv2.warpPerspective(input_img, matrix, (rows, rows))
		#_, result = cv2.threshold(result, 100, 255, cv2.THRESH_TOZERO)
		# This block of code is to enhance the borders of the whole maze so that it helps us to read the maze properly#
		# for the further application.
		#contours, _ = cv2.findContours(result, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
		#warped_img: None = cv2.drawContours(result, contours, -1, (0, 0, 0), 10)
	else:
		gray = cv2.cvtColor(input_img, cv2.COLOR_BGR2GRAY)
		_, gray = cv2.threshold(gray, 100, 255, cv2.THRESH_TOZERO)
		rows, col, ch = input_img.shape
		tx, ty = 0, 0
		bx, by = 0, col
		xtr, ytr = rows, 0
